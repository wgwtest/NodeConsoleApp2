...
- 例如“给目标增加 2 回合流血”，只需要 `buffRefs.apply` 指向 `buff_bleed`，并传入 `duration`/`extendBy`。

中复杂度：

- 技能动作读取指定 Buff 的层数、剩余回合或存在状态。
- 例如“按中毒层数增伤/回血”应使用 `BUFF_STACKS`。
- 例如“按流血剩余回合增伤/回血”应使用 `BUFF_REMAINING`。
- 必须确认 `amountSource.buffId` 指向存在的 Buff。
- 必须确认该 Buff 的状态模型匹配读数：流血是持续时间型，不能按层数设计；中毒这类状态才适合层数模型。
