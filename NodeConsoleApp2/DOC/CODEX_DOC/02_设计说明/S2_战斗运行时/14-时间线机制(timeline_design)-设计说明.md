...
- `script/engine/TurnPlanner.js`：规划器，输出“按 skill 维度”的规划结果（参考 `DOC/CODEX_DOC/02_设计说明/S4_技能系统与编辑器/05-技能规划(skill_planning_design)-设计说明.md`）
...
其中 `SkillPlan` 的字段应以 `DOC/CODEX_DOC/02_设计说明/S4_技能系统与编辑器/05-技能规划(skill_planning_design)-设计说明.md` 的输出为准（核心是“以 skill 为维度”而非 slot）。
