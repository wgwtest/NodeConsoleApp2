...
1. **Definition Layer（定义层）**
   - 定义 skill 的静态数据结构：id、target、costs、requirements、actions、buffRefs、unlock 等。
...
3. 在不改战斗逻辑的前提下，先识别“结构越界、目标选择失真、说明失真”这类配置层问题。
...
技能说明契约应优先校验正式字段之间的一致性，而不是依赖额外的持久化语义字段。
当前活动工程已经固定并长期回归的，是那些能直接从正式结构推出、且会真实影响规划与执行理解的问题码。
...
1. 设计文档、编辑器、技能树、回归工具能共享同一套正式字段词汇；
...
剑系不是“少量高价大技能”的路线，而是低 KP、低 AP、多节点、多动作的技巧型路线。

剑系的“花样繁多”应由技能树经济支撑：

1. 剑系节点数量可以多于锤系、护甲回复系等重型路线。
2. 单个低中阶节点 KP 消耗相对较低，让玩家前中期更容易横向学到多个小技能。
3. 同样的 KP 投入下，剑系应比锤系获得更多可用动作，但单个动作的数值上限更低。
4. 低阶剑系应允许玩家较早接触流血、连击、招架等入口，形成“技能很多、启动很快”的体验。
5. 高阶剑系仍应存在 KP 压力，避免玩家无成本买完整条剑系。
...
剑系的战斗数值仍必须遵循 `1 AP = 10 点即时 HP 伤害` 的基准。低 KP 只表示学习门槛低、工具数量多，不表示单技能可以突破 AP 伤害预算。

剑系应作为主角常用武器和技能树设计范例来打磨。它需要比其他路线更完整地覆盖以下能力：

1. 低阶小技能：保证前期招式多、启动快。
2. 流血与伤口：提供跨回合持续收益。
3. 连击与快攻：提供一回合多个动作的操作感。
4. 招架与防反：提供短期自保，防止被敌人 OTK 或快速压低血量。
5. 终结与爆发：提供适当的单轮巨大伤害，但必须是终结技或高约束技能。

剑系的爆发不能设计成低 KP、低 AP、无条件的大数字。合理约束包括：高 AP、高 KP、深层前置、目标低血、目标已有流血、消耗流血剩余回合、自损、占用关键部位槽、姿态窗口或每回合限制。

### 4.3.2 基于流血状态机的剑系组成建议

在 `buff_bleed` 被固定为“持续时间型状态”后，剑系不能再围绕“流血层数越叠越痛”设计。剑系的核心应改成：快速制造状态窗口、维持状态窗口、利用状态窗口，以及在高约束条件下兑现状态窗口。

当前正式技能包中的剑系骨架已经具备一条流血线：

1. `剑砍`：基础 1 AP / 10 HP 伤害入口。
2. `锯齿斩`：低阶流血启动器。
3. `撕裂伤口`：目标行动前延长流血持续回合。
4. `迸发`：按目标流血剩余回合造成伤害。
5. `吸血`：按目标流血剩余回合回复自身 HP。
6. `欠血还钱`：把流血构筑延伸为持续吸血。

这条骨架的问题也很明确：

1. 流血启动器太少，只有 `锯齿斩` 和 `撕裂伤口` 真正承担启动/延长职责。
2. 缺少“基础快剑”层次，导致低阶剑系只有一个直伤入口和一个流血入口。
3. 缺少招架、防反、自保窗口，剑系在高压敌人面前容易变成纯输出赌命路线。
4. 连击类技能如果只是“攻击 2 次/3 次/4 次”，没有触发价值、目标差异或槽位代价，就会变成换皮重复技能。
5. 终结技如果只按剩余回合读数但不消耗或限制状态窗口，后续容易变成低风险重复兑现。

推荐后续剑系按四类角色补齐，而不是继续堆相似伤害技能：

| 角色 | 设计目标 | 典型约束 |
| --- | --- | --- |
| 基础快剑 | 用低 AP 技能填充回合，提供稳定小伤害和部位选择 | 低 KP、低 AP、低数值，不附带高收益 |
| 流血操作 | 施加、延长、利用、必要时消耗流血剩余回合 | 读取 `BUFF_REMAINING`，不能读取流血层数 |
| 招架防反 | 提供短期自保和被攻击后的反打机会 | 持续短、每回合限制、占用手部或姿态窗口 |
| 终结爆发 | 在目标已有状态或低血时打出明显爽点 | 高 AP、高 KP、目标状态、低血条件、消耗流血窗口 |

后续新增剑系技能时，优先补“角色缺口”，不要补“描述不同但行为相同”的节点。特别是流血启动器必须区分：有的低伤低费只增加 1 回合流血；有的高费增加更多持续回合；有的通过撕裂状态延后增加持续回合；有的只在目标已流血时提供额外收益。
...
4. **技能读取 Buff 状态必须走通用读数，不写专用 Buff 逻辑**：
   - `BUFF_STACKS` 只用于中毒这类“层数型状态”，读取指定 `amountSource.buffId` 的 `stacks`。
   - `BUFF_REMAINING` 用于流血这类“持续时间型状态”，读取指定 `amountSource.buffId` 的 `remaining`。
   - 技能不得假设 `buff_bleed` 有伤害层数；流血只提供剩余回合数，伤害由 Buff 自身在回合结束固定结算。
   - 若某个技能要“兑现流血”，应写成“按目标流血剩余回合结算”，并通过 AP、KP、目标状态、每回合限制等方式约束强度。
...
- `duration`: 可选，表示本次施加的持续回合数；对 `stackStrategy: "extend"` 的持续时间型 Buff，可作为增加的回合数。
- `extendBy`: 可选，显式覆盖本次增加的持续回合数；适合“延长流血 N 回合”这类技能。
- `stackStrategy`: 可选，只能使用 Buff 系统已支持的通用策略；不能在技能侧写某个 Buff 的私有状态机。

---

### 5.2 技能分析维度与派生读数规范（补充）

本节将 06-技能平衡(skill_balance_design)-设计说明.md 中的“分析维度”融入技能设计方案，作为技能的工具侧派生语义层。
该层不改变引擎结算，也不写回活动技能数据，但能显著提升：技能库检索、强度对照、以及 editor / probe 的一致性校核能力。

#### 5.2.1 基本原则（正式字段 vs 派生读数）

- 技能字段（target、costs、requirements、actions、buffRefs）表达的是运行时事实。
- 分析维度表达的是工具侧派生语义，用于检索、统计、提示与平衡讨论。
- 两者冲突时，以正式字段为准；分析维度用于发现问题与提示修正。
- 派生结果不应写回活动 pack，也不应演化成第二套正式 schema。
...
5. 释放对象（Target Subject / Scope / Selection）
   ...
   - Selection：SELECT_FIXED_PART / SELECT_RANDOM_PART / SELECT_ALL_PARTS
...
#### 5.2.3 与运行时字段的对照（用于派生与校核）

下列对照用于编辑器校验、人工审查、分析页派生：技能当前的正式字段事实应该投影成哪些分析维度。

- SUBJECT_SELF 常见对应：target.subject = SUBJECT_SELF，或 legacy targetType=SELF/SELF_PARTS
- SUBJECT_ENEMY 常见对应：target.subject = SUBJECT_ENEMY，或 legacy targetType=ENEMY/SINGLE_PART/ALL_PARTS/RANDOM_PART
- SCOPE_ENTITY 常见对应：不要求具体部位；效果落在 HP/AP/speed 或全局 buff
- SCOPE_PART 常见对应：target.scope = SCOPE_PART，或 legacy targetType=SINGLE_PART
- SCOPE_MULTI_PARTS 常见对应：target.scope = SCOPE_MULTI_PARTS，或 legacy targetType=ALL_PARTS/SELF_PARTS
- SELECT_FIXED_PART 常见对应：target.selection.mode = single，且存在固定部位约束
- SELECT_RANDOM_PART 常见对应：target.selection.mode = random_single
- SELECT_ALL_PARTS 常见对应：target.selection.mode = multiple，且覆盖全部候选部位
...
- 正式字段：target.subject=SUBJECT_ENEMY、target.scope=SCOPE_PART、selection.mode=single
- 直接动作：effectType=DMG_ARMOR、amountType=ABS
- 工具侧读数可稳定派生：
  - DMG_ARMOR + ABS + INSTANT + ONE_SHOT
  - SUBJECT_ENEMY + SCOPE_PART + SELECT_FIXED_PART
  - 若存在部位槽消耗或来源部位要求，再追加 RES_SLOT / REQ_SELF_PART
...
> - 当前你已移除“流派”概念，因此模板中不再强制包含 `ARCH_*`。如未来需要扩展作者侧分类，可通过 groups、notes 或独立分析配置扩展。
...
```json
{
  "buffRefs": {
    "apply": [
      { "buffId": "buff_bleed", "target": "enemy", "chance": 1.0, "duration": 2, "stackStrategy": "extend" }
    ],
    "applySelf": [
...
对于持续时间型状态，`duration` 的含义是“本次施加给状态增加多少剩余回合”。例如 `buff_bleed` 施加 2 回合后，目标本回合结束会受到固定 5 点流血伤害，随后剩余回合减 1；再次施加 2 回合会继续增加剩余回合，而不是刷新或叠加伤害层数。

#### 5.3.6 派生分析读数（工具侧，不入库）

活动 skill schema 不再包含额外的持久化分析字段。
如果作者工具、平衡页或 probe 需要展示分析维度，应当在读取正式字段后实时派生，而不是把派生结果写回技能 pack。

工具侧可展示的派生读数，通常包括：

- 作用属性维度：从 actions[].effect.effectType 与 buffRefs 推导
- 数值类型维度：从 amountType、scaling、legacy valueType 推导
- 目标与范围维度：从 target.subject / target.scope / target.selection 推导
- 资源与限制维度：从 costs、requirements、perTurnLimit 推导
...
~~~json
{
  ...
  "target": {
    "subject": "SUBJECT_ENEMY",
    "scope": "SCOPE_PART",
    "selection": { "mode": "single", "selectCount": 1 }
  },
  "actions": [
    {
      "id": "a1",
...
| **Thousand Cuts (千刀风暴)** | `buff_bleed` | 多段命中判定，多次增加流血持续回合 |
| **Bloody Harvest (血腥收割)** | `buff_bleed` | “依赖目标已有流血”的条件判断（Buff 仍为 `buff_bleed`） |
| **Crimson Finale (猩红终结)** | `buff_bleed` | 结算/消耗流血剩余回合（移除动作不一定是 Buff，需要引擎支持 removeById；此处仅标明依赖 `buff_bleed`） |
