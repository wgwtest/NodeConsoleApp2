...
>
> `04-技能编辑器(skill_editor_design)-设计说明.md`、`05-技能规划(skill_planning_design)-设计说明.md`、`06-技能平衡(skill_balance_design)-设计说明.md`、`07-技能测试(skill_test_design)-设计说明.md` 和 `29-敌人技能包与技能编辑器模式切换(enemy_skill_pack_editor_mode)-设计说明.md` 是本文的子设计或关联设计文档。
...
4. **技能执行器消费通用结构。** 执行器只消费 `target / costs / requirements / actions / buffRefs`，不按具体技能名写特判。
...
- 定义战斗规划资源，包括 `costs.ap`、`costs.partSlot`、`costs.perTurnLimit`、`placement.maxSlots`。
- 定义 `actions[]` 的可执行效果集合。
...
- 历史旧技能附加字段继续作为正式 schema。这些由 `25-旧技能附加字段退役(skill_legacy_field_retirement)-设计说明.md` 说明退役口径。
...
| 部位槽资源 | `costs.partSlot`、slot layout、TurnPlanner | 规划阶段、战斗 UI |
...
| `requirements` | 是 | 是 | 释放门槛，不一定扣除资源 |
| `costs` | 是 | 是 | AP、部位槽、每回合限制等成本 |
| `placement` | 是/间接 | 是 | 技能槽或规划展示相关约束 |
| `speed` | 是 | 是 | 行动顺序修正 |
...
`requirements` 是释放门槛，表示必须满足但不一定扣除资源。

`costs` 是释放成本，表示规划或执行阶段需要扣除或占用的资源。

当前核心字段：

| 字段 | 类型 | 运行口径 |
| --- | --- | --- |
| `costs.ap` | 成本 | `_getSkillApCostStrict()` 要求为非负数字；执行后扣除 AP |
| `costs.partSlot` | 成本/占用 | 用于同回合部位槽位规划 |
| `costs.perTurnLimit` | 次数限制 | 用于规划限制；当前正式包每个技能均有该字段 |
| `requirements.selfPart` | 门槛 | 表示释放者某部位可用等要求 |
| `placement.maxSlots` | 编排约束 | 影响规划/槽位表现 |

设计规则：

1. `prerequisites` 只表达技能树前置关系。
2. `requirements` 表达战斗释放条件。
3. `costs` 表达战斗消耗或占用。
...
当前游戏设计不引入命中概率概念。多段动作是确定性重复结算，不是概率命中。
...
| `chance` | 当前代码兼容读取，但本项目当前设计不采用概率技能；新技能不推荐使用 |

旧文档中的非活动字段如果当前运行时不消费，不应继续作为推荐 schema。旧字段退役口径见 `25-旧技能附加字段退役(skill_legacy_field_retirement)-设计说明.md`。
...
| `isConditional` | `requirements` 非空 |
| `consumesPartSlot` | `costs.partSlot.part` |
...
- `chance` 字段当前代码兼容，但当前游戏设计没有概率命中或概率触发概念。新技能不应使用该字段，除非后续系统设计正式引入概率机制。
