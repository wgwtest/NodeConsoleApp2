...
2. **按结构类型归类。** 例如指定部位护甲伤害、明确 HP 伤害、治疗、护甲回复、Buff 施加、Buff 移除、Buff 读数计算、多段动作、自身目标、敌方部位目标。
...
`requirements` 是释放门槛的数据契约，表示设计上必须满足但不扣除资源。当前实现中，`DataManagerV2.getSkillContractSummary()` 会保留 `requirements` 并据此派生 `runtimeFlags.isConditional`，编辑器和 UI 会展示该字段；`CoreEngine._validateSkillRequirements()` 已在执行入口硬校验部分运行时条件。
...
| `requirements.targetHpPercentBelow` | 硬释放门槛 | `CoreEngine._validateSkillRequirements()` 校验目标当前 HP / 最大 HP 必须低于阈值 |
| `requirements.targetBuff` | 硬释放门槛 | `CoreEngine._validateSkillRequirements()` 校验指定 Buff 存在、`remaining` 或 `stacks` 达到门槛 |
| `placement.maxSlots` | 历史/编排字段 | 当前玩家技能包只有个别技能保留该字段；TurnPlanner 不消费它 |

设计规则：

1. `prerequisites` 只表达技能树前置关系。
2. `requirements` 表达战斗释放条件；只有本节列为硬释放门槛的字段可假设运行时已阻止非法释放，其它字段仍只是契约/展示。
...
4. `amountSource.maxRead` 是通用读取上限；当目标 Buff 的 `remaining/stacks` 高于该值时，只按 `maxRead` 参与计算。
5. 不消耗 Buff 资源的读数技能应优先设置 `maxRead`，避免长战斗中同一状态窗口被无限放大。
...
| `consumeRemaining` | 仅 `buffRefs.remove[]` 使用；消耗指定 Buff 的部分 `remaining`，消耗到 0 时移除该 Buff |
| `chance` | 当前代码兼容读取并会实际影响是否施加 Buff；当前玩家/敌人技能包仍存在 `chance < 1` 的数据。若继续坚持“无概率技能”设计，应把这些内容迁移为确定性约束或确定性效果 |

`buffRefs.remove[].consumeRemaining` 是通用窗口消费字段：

1. 未设置时，`buffRefs.remove` 表示移除整个指定 Buff。
2. 设置为正数时，只消耗该 Buff 的部分 `remaining`。
3. 如果消耗后 `remaining <= 0`，运行时移除该 Buff。
4. 该字段适合“消耗流血窗口造成爆发”这类 Skill-Buff 联动，不应为单个技能写专用引擎逻辑。

### 5.8 requirements.targetBuff

`requirements.targetBuff` 表示技能释放前必须检查某个实体身上的 Buff 状态。
...
| 字段 | 当前行为 |
| --- | --- |
| `buffId` | 必填，待检查 Buff ID |
| `minRemaining` | 要求目标 Buff 的 `remaining` 至少达到该值 |
| `minStacks` | 要求目标 Buff 的 `stacks` 至少达到该值 |
| `owner` | 可选；`self/actor` 检查释放者，`enemy/skillTarget` 或未设置时检查技能默认目标 |

该字段用于“目标正在流血”“目标至少有 3 回合流血”等战斗释放条件，不用于技能树前置。技能树结构仍由 `prerequisites` 表达。

### 5.9 requirements.targetHpPercentBelow

`requirements.targetHpPercentBelow` 表示目标当前 HP 百分比必须低于指定阈值才能释放。
...
{
  "requirements": {
    "targetHpPercentBelow": 0.35
  }
}
```

当前运行时由 `CoreEngine._validateSkillRequirements()` 校验：

```text
targetHpPercent = targetCurrentHp / targetMaxHp
可释放条件：targetHpPercent < targetHpPercentBelow
```

该字段适合“斩杀”“终结技”“低血压制”等战斗释放门槛。它不表达技能树前置，也不替代 `requirements.targetBuff` 这类状态门槛。

旧文档中的非活动字段如果当前运行时不消费，不应继续作为推荐 schema。旧字段退役口径以本文第 12 章为准。

### 5.10 unlock 与 prerequisites
...
| 类型 | 触发入口 | 运行时路径 | 典型字段 |
| --- | --- | --- | --- |
| 指定部位护甲伤害 | 技能执行 | `_executeSkillActions -> DMG_ARMOR` | `actions[].effect.effectType=DMG_ARMOR` |
| 明确 HP 伤害 | 技能执行 | `_executeSkillActions -> DMG_HP` | `DMG_HP` |
...
1. 基础伤害公式以 `1 AP = 10 点指定部位护甲伤害` 为基本量尺。
