...
当前共有 `3` 个可直接人工验收节点，另有 `2` 个节点需先重写验收清单再进入人工验收。
...
1. `#89 / WBS-4.1.3`
   ...
   - 说明：本节点把战斗页、游戏菜单、关卡选择页的关键信息收敛到统一摘要骨架，当前人工验收重点是摘要区是否真的帮助理解“现在该做什么”，同时不再压缩主画面。
2. `#98 / WBS-3.3.3`
   - 状态：`待验收`
   - 清单：
   ...
   - 说明：本节点用于把技能异常正式纳入“技能状态总表 + 分批修复 + 状态回写”闭环，当前人工验收重点是总表是否先回答技能状态、首批结构异常是否已收口，以及 `skill tags` 后续分析是否已独立拆出。
3. `#97 / WBS-4.3.7`
   - 状态：`待验收`
   - 清单：
   ...
   - 说明：本节点把对战展示参数提升为正式配置资产，并提供独立配置器、probe 读数与主流程消费入口；当前人工验收重点是配置器是否已形成同屏编辑 / 预览工作区，以及展示层是否已去掉整屏位移晃动。
