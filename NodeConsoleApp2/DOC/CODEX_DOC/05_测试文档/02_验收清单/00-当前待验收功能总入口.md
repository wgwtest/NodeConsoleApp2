...
1. 本轮新待验收项 `C1.1 ~ C1.3` 已加入本页集中入口。
2. 本轮新增 `D3.1`，用于验证对战展示层已经能在 probe 页和主流程页同时成立。
3. 本轮新增 `D3.2`，用于验证主工程已经把与 `SpineAssets` 的协作规则、消费边界和降级语义固定下来。
4. 已通过节点仍继续保留在“已同步移出节点”列表中，不重新回流。
