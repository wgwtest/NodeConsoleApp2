...
function getAnimationKeyframesBlock(css, selectorPattern) {
    const animationMatch = css.match(selectorPattern);
    assert.ok(animationMatch, `缺少动画配置：${selectorPattern}`);
...
test('battle_presentation_configurator 的实时预览应以角色场景为主，不再用完整 HUD 挤压角色区', () => {
    const html = readUtf8(configuratorPath);

    assert.match(html, /实时预览/, '配置器缺少实时预览区');
    assert.doesNotMatch(html, /class="hud-panel player-hud labeled-block"/, '配置器预览仍保留完整玩家 HUD');
    assert.doesNotMatch(html, /class="hud-panel enemy-hud labeled-block"/, '配置器预览仍保留完整敌人 HUD');
    assert.match(html, /preview-status-strip/, '配置器缺少精简状态条容器');
});

test('battle-scene 的 scene-impact 不再通过位移制造屏幕晃动', () => {
    const css = readUtf8(mockCssPath);
    const keyframesBlock = getAnimationKeyframesBlock(
        css,
        /\.battle-scene\.scene-impact\s*\{[^}]*animation:\s*([a-zA-Z0-9_-]+)/
    );

    assert.doesNotMatch(keyframesBlock, /translate\(/, 'scene-impact 仍通过 translate 触发屏幕晃动');
});

test('battle-scene 的 scene-beat-self 与 scene-beat-enemy 不再推动整块场景横向位移', () => {
    const css = readUtf8(mockCssPath);
    const selfKeyframes = getAnimationKeyframesBlock(
        css,
        /\.battle-scene\.scene-beat-self\s*\{[^}]*animation:\s*([a-zA-Z0-9_-]+)/
    );
    const enemyKeyframes = getAnimationKeyframesBlock(
        css,
        /\.battle-scene\.scene-beat-enemy\s*\{[^}]*animation:\s*([a-zA-Z0-9_-]+)/
    );

    assert.doesNotMatch(selfKeyframes, /translateX\(/, 'scene-beat-self 仍推动整块场景横向位移');
    assert.doesNotMatch(enemyKeyframes, /translateX\(/, 'scene-beat-enemy 仍推动整块场景横向位移');
});
