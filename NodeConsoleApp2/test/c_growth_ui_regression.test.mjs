...
test('UI_SystemModal 主菜单会显示最近成长来源与最近学习结果', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(bodyText, /进入关卡前先确认/);
    assert.doesNotMatch(bodyText, /构筑方向/);
...
test('UI_SystemModal 主菜单不再显示内容入口解释说明', async () => {
  const dom = createSystemModalFixture();
  try {
    const { UI_SystemModal } = await importSourceModule('script/ui/UI_SystemModal.js');
    const modal = new UI_SystemModal();
    ...
    assert.doesNotMatch(bodyText, /内容入口说明/);
    assert.doesNotMatch(bodyText, /作者样本工具页/);
    assert.doesNotMatch(bodyText, /不属于主流程/);
    assert.doesNotMatch(bodyText, /地图编辑页已支持逻辑空间与显示配置编辑/);
    assert.equal(document.querySelector('#modalBody [data-summary-kind="content-sources"]'), null);
  } finally {
    dom.window.close();
    cleanupDomGlobals();
  }
...
test('UI_SystemModal 关卡选择页保留章节推进信息并移除关前构筑说明', async () => {
  const dom = createSystemModalFixture();
  try {
    const { UI_SystemModal } = await importSourceModule('script/ui/UI_SystemModal.js');
    const modal = new UI_SystemModal();
    ...
    const bodyText = document.getElementById('modalBody')?.textContent || '';
    assert.match(bodyText, /章节地图/);
    assert.match(bodyText, /当前推荐/);
    assert.match(bodyText, /密林前哨/);
    assert.match(bodyText, /1-2/);
    assert.equal(document.querySelector('#modalBody .level-select-map-slot[data-summary-kind="story-progress"]'), document.querySelector('#modalBody [data-summary-kind="story-progress"]'));
    assert.equal(document.querySelector('#modalBody details.level-card-extra-details'), null);
    assert.doesNotMatch(bodyText, /章节节点|推进关系|重复通关收益|更多信息/);
    assert.doesNotMatch(bodyText, /关前构筑摘要/);
    assert.doesNotMatch(bodyText, /预装配说明/);
    assert.doesNotMatch(bodyText, /本版本无单独预装配步骤/);
    assert.equal(document.querySelector('#modalBody [data-summary-kind="prebattle-build"]'), null);
  } finally {
    dom.window.close();
    cleanupDomGlobals();
  }
