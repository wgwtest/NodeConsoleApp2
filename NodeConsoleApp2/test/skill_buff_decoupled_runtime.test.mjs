...
test('BuffRegistry 会解析 paramsSchema 默认值和覆盖值，不需要技能侧解释模板', async () => {
  ...
  const bleed = registry.getDefinition('buff_bleed', {
    params: { buff_duration: 4 }
  });
  assert.equal(bleed.lifecycle.duration, 4);
  assert.equal(bleed.lifecycle.maxStacks, 1);
  assert.equal(bleed.lifecycle.stackStrategy, 'extend');
  assert.equal(bleed.effects[0].trigger, 'onTurnEnd');
...
test('BuffManager 能按持续时间型状态增加 remaining、按回合过期，并提供通用属性修正', async () => {
  const runtime = await buildRuntime();
  const actor = attachBuffs(createActor('player_1', { speed: 10 }), runtime);

  actor.buffs.add('buff_bleed', { params: { buff_duration: 2 } });
  assert.equal(actor.buffs.getRemaining('buff_bleed'), 2);
  assert.equal(actor.buffs.getStacks('buff_bleed'), 1);
  actor.buffs.tickTurn();
  assert.equal(actor.buffs.getAll()[0].remaining, 1);

  actor.buffs.add('buff_bleed', { params: { buff_duration: 2 } });
  assert.equal(actor.buffs.getAll()[0].remaining, 3);
  assert.equal(actor.buffs.getRemaining('buff_bleed'), 3);
  assert.equal(actor.buffs.getStacks('buff_bleed'), 1);
...
test('BuffSystem 能通过通用回合事件触发流血、中毒和晕眩', async () => {
  ...
  assert.equal(actor.buffs.getRemaining('buff_bleed'), 1);
  assert.equal(actor.buffs.has('buff_bleed'), true);

  runtime.eventBus.emit('TURN_END', { turn: 2 });
  assert.equal(actor.stats.hp, 76);
  assert.equal(actor.buffs.has('buff_bleed'), false);
...
test('BUFF_REMAINING 是通用读数，可读取任意持续时间型 Buff 的 remaining', async () => {
  const { BuffRegistry, BuffManager, BuffSystem } = await importBuffRuntime();
  const { CoreEngine } = await importCoreEngineClass();
  const eventBus = new TestEventBus();
  const registry = new BuffRegistry({
    buff_window_probe: {
      id: 'buff_window_probe',
      name: '窗口探针',
      type: 'debuff',
  ...
  driver.data = { playerData: player, currentLevelData: { enemies: [enemy] } };
  enemy.buffs.add('buff_window_probe', { duration: 3 });

  const result = driver._executeSkillActions({
    actor: player,
    action: { targetId: enemy.id, bodyPart: 'chest' },
    skillConfig: {
      id: 'skill_buff_remaining_probe',
      name: '持续窗口探针',
...
test('标准半重构剑系候选包的普通命中优先扣护甲，护甲充足时不扣 HP', async () => {
  const rawSkills = JSON.parse(await fs.readFile(
...
test('DMG_ARMOR 会先扣指定部位护甲，护甲不足的部分溢出为 HP 伤害', async () => {
  const runtime = await buildRuntime();
  const { CoreEngine } = await importCoreEngineClass();
  const player = attachBuffs(createActor('player_1', { ap: 5 }), runtime);
  const enemy = attachBuffs(createActor('enemy_1', {
    hp: 50,
    maxHp: 50,
    bodyParts: {
  ...
  const skill = {
    id: 'skill_armor_overflow_probe',
    name: '护甲溢出探针',
    target: { subject: 'SUBJECT_ENEMY', scope: 'SCOPE_PART' },
    costs: { ap: 1 },
    actions: [
      {
        id: 'action_armor_overflow_probe',
        effect: { effectType: 'DMG_ARMOR', amountType: 'ABS', amount: 16 }
      }
    ]
  };
  const driver = Object.create(CoreEngine.prototype);
  ...
  assert.equal(result.ok, true);
  assert.equal(result.actions[0].armorDamage, 6);
  assert.equal(result.actions[0].damage, 10);
  assert.equal(enemy.bodyParts.chest.current, 0);
  assert.equal(enemy.stats.hp, 40);
});
...
test('撕裂伤口施加撕裂状态，撕裂状态在行动前通过 Buff 侧逻辑增加流血持续回合', async () => {
  ...
  assert.equal(tearCase.enemy.buffs.getRemaining('buff_bleed'), 2);
  ...
  assert.equal(tearCase.enemy.buffs.getStacks('buff_bleed'), 1);
  assert.equal(tearCase.enemy.buffs.getRemaining('buff_bleed'), 4);
