...
test('mock_ui_v11.css 为关卡选择提供 1920 首屏验收所需的宽面板布局', async () => {
  ...
  assert.doesNotMatch(css, /\.level-map-edge\s*\{[^}]*stroke-dasharray:/s);
  assert.doesNotMatch(css, /\.level-map-edge\.is-/s);
  ...
  assert.match(css, /\.level-map-edge\[data-walked="true"\]\s*\{[^}]*stroke:\s*rgba\(112,\s*82,\s*42,/s);
  assert.doesNotMatch(css, /\.level-map-edge\[data-active="true"\]/s);
