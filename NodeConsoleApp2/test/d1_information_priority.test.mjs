...
test('mock_ui_v11.css 为关卡选择提供 1920 首屏验收所需的宽面板布局', async () => {
  ...
  assert.match(css, /\.level-map-node__plate\s*\{[^}]*grid-template-columns:\s*42px\s+minmax\(0,\s*1fr\)/s);
  assert.match(css, /\.level-map-node\[data-selected="true"\]\s+\.level-map-node__plate\s*\{[^}]*box-shadow:[^}]*0\s+0\s+0\s+3px\s+rgba\(255,\s*218,\s*107,/s);
  assert.match(css, /\.level-map-node\[data-selected="true"\]\s+\.level-map-node__plate::after\s*\{[^}]*background:\s*rgba\(255,\s*218,\s*107,/s);
