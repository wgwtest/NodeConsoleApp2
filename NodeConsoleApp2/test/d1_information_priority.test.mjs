...
test('UI_SystemModal 会优先渲染运行时地图包节点并允许从地图选择关卡', async () => {
  const dom = createSystemModalFixture();
  try {
    ...
    assert.deepEqual(selectedLevels, ['level_1_2_story']);
  } finally {
    dom.window.close();
    cleanupDomGlobals();
  }
...
test('mock_ui_v11.css 为关卡选择提供 1920 首屏验收所需的宽面板布局', async () => {
  ...
  assert.match(css, /\.modal-panel--level-select\s*\{[^}]*width:\s*min\(1500px,\s*calc\(100vw - 160px\)\)/s);
  assert.match(css, /\.modal-body--level-select\s*\{[^}]*overflow:\s*hidden/s);
  assert.match(css, /\.level-select-layout\s*\{[^}]*grid-template-columns:\s*340px\s+minmax\(0,\s*1fr\)/s);
