...
const runtimeEnemyDocumentPath = 'assets/enemy_packs/current/enemies.json';
...
test('关卡事实源定义 30 个 story 关卡、6 个路线变体和独立敌人池', async () => {
  const enemyIds = new Set(Object.keys(normalizeEnemies(await readJson(runtimeEnemyDocumentPath))));
...
test('三章 30 关敌人模板满足 WBS-3.4.2 的数量、角色和字段完整性约束', async () => {
  const enemies = normalizeEnemies(await readJson(runtimeEnemyDocumentPath));
  ...
  assert.equal(enemyEntries.length, 30, 'V2 current enemy pack should publish one main enemy for each formal story level');
...
test('WBS-3.4.7 第三章 P0 关卡应使用章节专属敌人替换旧样本补位', async () => {
  ...
  for (const { relativePath, document } of levelDocuments) {
    ...
    for (const [levelId, expected] of Object.entries(expectedP0Enemies)) {
      ...
      assert.equal(
        /^(enemy|boss)_v2_c3_/u.test(expected.enemyId),
        true,
...
test('三章 30 关实际使用敌人的技能组满足 WBS-3.4.3 的数量、AP 和首回合行动约束', async () => {
  ...
  for (const enemyId of usedEnemyIds) {
    ...
    if (tags.includes('role_boss') && skills.length !== 4) {
      skillCountIssues.push(`${enemyId}: expected boss 4, got ${skills.length}`);
    } else if (tags.includes('role_elite') && (skills.length < 2 || skills.length > 4)) {
      skillCountIssues.push(`${enemyId}: expected elite 2-4, got ${skills.length}`);
    } else if (!tags.includes('role_boss') && !tags.includes('role_elite') && (skills.length < 2 || skills.length > 3)) {
...
test('终局后期吸血应早于暮堡重抡形成可执行反制窗口', async () => {
  ...
  const bossBaseSpeed = Number(enemies.boss_v2_c3_10_twilight_bastion?.stats?.speed ?? 0) || 0;
