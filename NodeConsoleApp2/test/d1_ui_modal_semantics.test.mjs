...
test('UI_SystemModal 欢迎页会解释新游戏、读取存档与主流程顺序', async () => {
  const dom = createSystemModalFixture();
  try {
    const { UI_SystemModal } = await importSourceModule('script/ui/UI_SystemModal.js');
    const modal = new UI_SystemModal();
    ...
    const bodyText = document.getElementById('modalBody')?.textContent || '';
    const kinds = Array.from(document.querySelectorAll('#modalBody .summary-section'))
      .map(node => node.getAttribute('data-summary-kind'));

    assert.match(bodyText, /主流程怎么走/);
    assert.match(bodyText, /新游戏.*自动存档.*游戏菜单/);
    assert.match(bodyText, /读取存档.*直接恢复到存档对应的页面/);
    assert.match(bodyText, /关卡选择.*提交规划.*执行回合/);
    assert.ok(kinds.includes('main-flow'), '欢迎页缺少 main-flow 摘要语义类');
  } finally {
    dom.window.close();
    cleanupDomGlobals();
  }
...
test('UI_SystemModal 主菜单会解释正式主流程入口与技能树子流程分工', async () => {
  const dom = createSystemModalFixture();
  try {
    const { UI_SystemModal } = await importSourceModule('script/ui/UI_SystemModal.js');
    const modal = new UI_SystemModal();
    ...
    assert.match(bodyText, /主流程怎么走/);
    assert.match(bodyText, /关卡选择.*当前正式主流程入口/);
    assert.match(bodyText, /技能树\s*\/\s*构筑.*主流程关联子流程/);
    assert.match(bodyText, /选择技能并部署到技能槽.*提交规划.*执行回合/);
    assert.ok(kinds.includes('main-flow'), '主菜单缺少 main-flow 摘要语义类');
  } finally {
    dom.window.close();
    cleanupDomGlobals();
  }
