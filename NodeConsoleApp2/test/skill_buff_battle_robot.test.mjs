...
test('战斗机器人按真实回合流验证撕裂伤口、迸发、吸血与流血持续回合结算', async () => {
  ...
  assert.equal(enemy.stats.hp, 35);
  ...
  assert.equal(enemy.stats.hp, 25);
  ...
  assert.equal(burstResult?.actions?.[0]?.targetHpRemaining, 20);
  assert.equal(enemy.buffs.getStacks('buff_bleed'), 1);
  assert.equal(enemy.buffs.getRemaining('buff_bleed'), 2);
  assert.equal(enemy.hp, 15);
  assert.equal(enemy.stats.hp, 15);
  ...
  assert.equal(enemy.stats.hp, 10);
