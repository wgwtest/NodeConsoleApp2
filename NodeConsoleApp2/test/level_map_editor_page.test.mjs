...
function createPageFixture() {
    const dom = new JSDOM(`
        ...
        <body>
            <div id="status"></div>
            <button id="newPackageBtn" type="button">new package</button>
            <button id="newPackageInlineBtn" type="button">new package inline</button>
...
test('LevelMapEditorPage 渲染单故事包的章节地图层级并支持新建复制删除地图', async () => {
    const dom = createPageFixture();
    try {
        const { page } = await createPageContext();
        await page.loadDefaultDocuments();

        assert.match(document.getElementById('storyList').textContent || '', /一个地图包对应一个故事/u, '左侧缺少当前故事包信息');
...
test('LevelMapEditorPage 支持新建故事包并生成首个章节地图', async () => {
    const dom = createPageFixture();
    try {
        const { page } = await createPageContext();
        await page.loadDefaultDocuments();

        document.getElementById('packageIdInput').value = 'new_story_pack';
        document.getElementById('packageTitleInput').value = '新故事包';
        document.getElementById('newPackageInlineBtn').click();
        ...
        assert.equal(exported.stories.length, 1, '新建地图包应创建唯一故事');
        assert.equal(exported.chapters.length, 1, '新建地图包应带首个章节');
        assert.equal(exported.maps.length, 1, '新建地图包应带首张地图');
        assert.equal(exported.stories[0].chapterIds.includes(exported.chapters[0].id), true);
        assert.equal(exported.chapters[0].mapIds.includes(map.id), true);
        assert.equal(map.nodes.length >= 1, true, '首张地图应有可编辑入口节点');
        assert.equal(map.entryNodeId, map.nodes[0].id);
        assert.equal(page.selectedStoryId, exported.stories[0].id);
        assert.equal(page.selectedChapterId, exported.chapters[0].id);
        assert.equal(page.selectedMapId, map.id);
        assert.match(document.getElementById('storyList').textContent || '', /新故事包/u);
        assert.match(document.getElementById('status').textContent || '', /已新建故事包/u);
    } finally {
        dom.window.close();
        cleanupDomGlobals();
    }
...
test('LevelMapEditorPage 将地图包视为单一故事，不在包内新建第二个故事', async () => {
    const dom = createPageFixture();
    try {
        const { page } = await createPageContext();
        await page.loadDefaultDocuments();

        const before = JSON.parse(document.getElementById('exportPreview').value);
        assert.equal(before.stories.length, 1, '一个地图包应只承载一个故事');

        document.getElementById('storyTitleInput').value = '错误的新故事';
        document.getElementById('addStoryBtn').click();

        const after = JSON.parse(document.getElementById('exportPreview').value);
        assert.equal(after.stories.length, 1, '新建故事不应在当前地图包内追加第二个故事');
        assert.equal(after.stories[0].id, before.stories[0].id);
        assert.match(document.getElementById('storyList').textContent || '', /一个地图包对应一个故事/u);
    } finally {
        dom.window.close();
        cleanupDomGlobals();
    }
...
test('LevelMapEditorPage 在章节增长时会自适应放大章节列表并保留内部滚动', async () => {
    const dom = createPageFixture();
    try {
        const { page } = await createPageContext();
        await page.loadDefaultDocuments();

        for (const title of ['霜雾峡谷', '断桥古道', '暮影回廊', '灰烬高地', '遗迹前庭']) {
        ...
        assert.equal(exported.chapters.length >= 6, true, '章节增长后应仍保留全部章节');
        assert.equal(chapterList.style.maxHeight, '192px');
        assert.equal(chapterList.style.overflowY, 'auto');
        assert.equal(chapterList.querySelectorAll('button').length >= 6, true, '章节列表应保留所有章节条目');
    } finally {
        dom.window.close();
        cleanupDomGlobals();
    }
...
test('LevelMapEditorPage 支持新增节点、编辑边并更新位置', async () => {
    ...
    try {
        ...
        assert.equal(Object.hasOwn(edge, 'type'), false);
...
test('LevelMapEditorPage 导出目录包会使用可编辑包目录并生成完整地图包文件', async () => {
    const dom = createPageFixture();
    try {
        const { page } = await createPageContext();
        await page.loadDefaultDocuments();
        ...
        assert.deepEqual(files.map(file => file.fileName), ['package.json', 'maps.json', 'levels.json', 'asset-manifest.json']);
        ...
        assert.equal(JSON.parse(files[2].content).$schemaVersion, 'levels_v1_wrapped');
        assert.equal(Array.isArray(JSON.parse(files[3].content).backgrounds), true);
...
test('LevelMapEditorPage 支持选择目录后写入完整目录式地图包', async () => {
    const dom = createPageFixture();
    try {
        const { page } = await createPageContext();
        await page.loadDefaultDocuments();
        ...
        assert.deepEqual(Object.keys(writtenFiles).sort(), ['asset-manifest.json', 'levels.json', 'maps.json', 'package.json']);
        assert.equal(JSON.parse(writtenFiles['package.json']).packageId, 'custom_story_pack');
        assert.equal(JSON.parse(writtenFiles['package.json']).files.levels, 'levels.json');
        assert.equal(JSON.parse(writtenFiles['maps.json']).$schemaVersion, 'level_map_pack_v1');
        assert.equal(JSON.parse(writtenFiles['levels.json']).$schemaVersion, 'levels_v1_wrapped');
...
test('LevelMapEditorPage 保存工作稿和发布会把关卡定义写入地图包目录', async () => {
    const dom = createPageFixture();
    try {
        const { page } = await createPageContext();
        await page.loadDefaultDocuments();
        ...
        assert.deepEqual(writes[0].body.files.map(file => file.fileName), ['package.json', 'maps.json', 'levels.json', 'asset-manifest.json']);
        assert.deepEqual(writes[1].body.files.map(file => file.fileName), ['package.json', 'maps.json', 'levels.json', 'asset-manifest.json']);
        assert.equal(JSON.parse(writes[0].body.files[0].content).packageId, 'story_pack_v1');
        assert.equal(JSON.parse(writes[0].body.files[0].content).files.levels, 'levels.json');
        assert.equal(JSON.parse(writes[1].body.files[1].content).$schemaVersion, 'level_map_pack_v1');
        assert.equal(JSON.parse(writes[0].body.files[2].content).$schemaVersion, 'levels_v1_wrapped');
        assert.equal(JSON.parse(writes[1].body.files[2].content).enemyPools.pool_story_goblin_edge.members[0].templateId, 'goblin_story_headhunter');
    } finally {
        dom.window.close();
        cleanupDomGlobals();
    }
...
test('LevelMapEditorPage 保存工作稿接口不可用时给出可执行的服务启动提示', async () => {
    const dom = createPageFixture();
    try {
        const { page } = await createPageContext();
        await page.loadDefaultDocuments();
...
test('level_map_editor_v1.html 是紧凑地图编辑工作台，保留可收起抽屉并移除说明页文案', async () => {
    ...
    assert.match(html, /id="newPackageBtn"/u, '地图工程操作必须提供新建故事包入口');
    ...
    assert.doesNotMatch(html, /id="edgeTypeSelect"/u, '边检查器不应再管理 branch/merge 类型');
    assert.doesNotMatch(html, /data-edge-type/u, '编辑器画布不应再把边类型写入 DOM');
    assert.doesNotMatch(html, /\.map-edge-group\[data-edge-type=/u, '编辑器 CSS 不应再按边类型区分样式');
    assert.doesNotMatch(html, /stroke-dasharray:/u, '编辑器连线应统一使用实线');
    assert.doesNotMatch(html, /分支文本/u, '边标签文案不应继续暗示分支类型');
    assert.match(html, /边标签/u, '边详情应保留普通边标签编辑');
    assert.match(html, /id="storyList" class="list-box story-list-box"/u, '左侧应先提供当前故事包信息');
    ...
    const leftListBlock = html.match(/\.story-list-box\s*,\s*\n\s*\.chapter-list-box\s*,\s*\n\s*\.map-list-box\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(leftListBlock, /max-height:\s*clamp\(96px,\s*14vh,\s*180px\)/u, '左侧结构列表应随视口动态约束高度');
    assert.match(leftListBlock, /overflow-y:\s*auto/u, '左侧结构列表新增后应在列表内部滚动');
    assert.doesNotMatch(leftListBlock, /max-height:\s*112px/u, '左侧结构列表不应继续使用固定 112px 死高度');
        assert.doesNotMatch(html, /id="addStoryBtn"/u, '包内不应提供新建故事操作');
        assert.doesNotMatch(html, /aria-label="故事操作"/u, '包内不应提供故事操作区');
        assert.match(html, /id="newPackageInlineBtn"/u, '左侧故事包区应提供显眼的新建故事包入口');
    ...
    assert.doesNotMatch(html, /id="inspectorModeLabel"[^>]*class="badge/u, '右侧不应显示 node/edge 模式徽标浪费空间');
    const srOnlyBlock = html.match(/\.sr-only\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(srOnlyBlock, /position:\s*absolute/u, '模式状态只能给辅助技术读取，不能占用右侧标题空间');
    assert.match(srOnlyBlock, /width:\s*1px/u, '模式状态不能形成可见文本');
    assert.match(html, /class="drawer-title inspector-head"/u, '右侧标题和检查器 tab 应合并到同一行');
    assert.match(html, /id="nodeList" class="list-box inspector-scroll-list node-list-box"/u, '右侧节点列表应有独立滚动高度策略');
    assert.match(html, /class="section-block inspector-list-block edge-list-block"/u, '边列表应使用独立滚动列表块，不能和详情粘在一起');
    assert.match(html, /class="section-block selected-detail edge-detail-block"/u, '边详情应和边列表分区，避免互相挤压');
    ...
    const inspectorHeadBlock = html.match(/\.inspector-head\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(inspectorHeadBlock, /display:\s*grid/u, '右侧标题和 tab 应紧凑并列，不能上下堆叠浪费高度');
    assert.match(inspectorHeadBlock, /grid-template-columns:\s*auto\s+minmax\(0,\s*1fr\)/u, '检查器 tab 应和标题在同一行');
    const inspectorSummaryBlock = html.match(/\.inspector-summary\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(inspectorSummaryBlock, /min-height:\s*30px/u, '当前节点/边摘要应压成紧凑行');
    const inspectorActionRowBlock = html.match(/\.inspector-action-row\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(inspectorActionRowBlock, /display:\s*grid/u, '检查器操作按钮应稳定排布，避免换行挤压列表');
    assert.match(inspectorActionRowBlock, /grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\)/u, '检查器操作按钮应使用单行紧凑网格，避免挤压列表');
    const inspectorScrollListBlock = html.match(/\.inspector-scroll-list\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(inspectorScrollListBlock, /min-height:\s*clamp\(180px,\s*24vh,\s*280px\)/u, '节点和边列表都必须保留可用高度，不能被详情挤成几行');
    assert.match(inspectorScrollListBlock, /overflow-y:\s*auto/u, '节点和边列表都应使用内部滚动');
    const inspectorScrollButtonBlock = html.match(/\.inspector-scroll-list\s+button\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(inspectorScrollButtonBlock, /min-height:\s*32px/u, '节点和边列表条目都应有稳定高度，不能粘在一起');
    const inspectorPanelBlock = html.match(/\.inspector-panel\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(inspectorPanelBlock, /display:\s*grid/u, '检查器面板应使用网格分配列表和详情空间');
    assert.match(inspectorPanelBlock, /grid-template-rows:\s*minmax\(300px,\s*0\.85fr\)\s+minmax\(0,\s*1fr\)/u, '检查器列表应保留基础高度，详情用剩余空间滚动');
    const rightDrawerBlock = html.match(/\.drawer--right\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(rightDrawerBlock, /overflow-y:\s*hidden/u, '右侧检查器抽屉不应整体滚动，列表和详情应各自滚动');
    const rightDrawerShellBlock = html.match(/\.drawer--right\s+\.drawer-shell\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(rightDrawerShellBlock, /height:\s*100%/u, '右侧检查器内部必须被抽屉高度约束，避免边列表撑开整栏');
    assert.match(rightDrawerShellBlock, /min-height:\s*0/u, '右侧检查器内部必须允许面板收缩出滚动区域');
    const inspectorListBlock = html.match(/\.inspector-list-block\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(inspectorListBlock, /min-height:\s*0/u, '列表块必须允许内部滚动收缩');
    assert.match(inspectorListBlock, /overflow:\s*hidden/u, '列表块应约束内容，滚动交给列表本身');
    const edgeListBlock = html.match(/\.edge-list-box\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(edgeListBlock, /max-height:\s*none/u, '边列表不能继续用固定 360px 挤压详情');
    const edgeDetailBlock = html.match(/\.edge-detail-block\s*\{(?<css>[\s\S]*?)\n\s*\}/u)?.groups?.css || '';
    assert.match(edgeDetailBlock, /max-height:\s*clamp\(260px,\s*38vh,\s*420px\)/u, '边详情应限制高度并独立滚动');
