 ...
 */
function stripDuplicatedSkillCopySuffix(skillId) {
...
function buildDuplicatedSkillId(skillId, seed = Date.now()) {
...
class SkillEditor {
    constructor() {
        ...
        this.elPropTargetType = document.getElementById('prop-targetType');
        this.elPropRequiredPart = document.getElementById('prop-requiredPart');
        // targetParts dropdown
        this.elTargetPartsBtn = document.getElementById('targetPartsBtn');
        this.elTargetPartsDropdown = document.getElementById('targetPartsDropdown');
        this.elPropType = document.getElementById('prop-type');
        this.elPropValue = document.getElementById('prop-value');
        this.elPropValueType = document.getElementById('prop-valueType');
        ...
        // TargetParts dropdown interactions
        this.elTargetPartsBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (this.elTargetPartsDropdown.style.display === 'none') {
                this.elTargetPartsDropdown.style.display = 'block';
            } else {
                this.elTargetPartsDropdown.style.display = 'none';
            }
        });

        document.getElementById('targetPartsClose').addEventListener('click', (e) => {
            e.preventDefault();
            this.elTargetPartsDropdown.style.display = 'none';
            this.updateTargetPartsBtnText();
        });
        document.getElementById('targetPartsAll').addEventListener('click', (e) => {
            e.preventDefault();
            this.setTargetPartsSelection(['head','chest','left_arm','right_arm','left_leg','right_leg']);
        });
        document.getElementById('targetPartsNone').addEventListener('click', (e) => {
            e.preventDefault();
            this.setTargetPartsSelection([]);
        });

        // Click outside closes
        window.addEventListener('mousedown', (e) => {
            if (!this.elTargetPartsDropdown) return;
            if (this.elTargetPartsDropdown.style.display !== 'block') return;
            const inside = this.elTargetPartsDropdown.contains(e.target) || this.elTargetPartsBtn.contains(e.target);
            if (!inside) {
                this.elTargetPartsDropdown.style.display = 'none';
                this.updateTargetPartsBtnText();
            }
        ...
        // Checkbox changes update button label
        this.elTargetPartsDropdown.addEventListener('change', () => {
            this.updateTargetPartsBtnText();
        });
    }
    ...
    loadProperties(skill) {
        ...
        if (!skill) {
            ...
            this.elPropCost.value = '';
            this.elPropSpeed.value = '';
            this.elPropDesc.value = '';
            this.elPropEffects.value = '';
        ...
        this.elPropCost.value = skill.cost ?? 0;
        this.elPropSpeed.value = skill.speed ?? 0;
        this.elPropTargetType.value = skill.targetType || 'SINGLE_PART';
        this.elPropRequiredPart.value = skill.requiredPart || '';
        this.setTargetPartsSelection(Array.isArray(skill.targetParts) ? skill.targetParts : []);
        this.elPropType.value = skill.type || '';
        this.elPropValue.value = skill.value ?? '';
        this.elPropValueType.value = skill.valueType || '';
        this.elPropDesc.value = skill.description || '';
        this.elPropEffects.value = JSON.stringify(skill.effects || [], null, 2);
    ...
    saveEffectsJson() {
        ...
        let effects = [];
        try {
            effects = JSON.parse(this.elPropEffects.value || '[]');
            if (!Array.isArray(effects)) throw new Error('effects 必须是数组');
            // minimal schema:
            // - legacy: effects[i].type
            // - v3: effects[i].effectType
            for (let i = 0; i < effects.length; i++) {
                const eff = effects[i];
                if (typeof eff !== 'object' || eff === null || Array.isArray(eff)) {
                    throw new Error(`effects[${i}] 必须是对象`);
                }
                const ok = (typeof eff.type === 'string' && eff.type.trim()) || (typeof eff.effectType === 'string' && eff.effectType.trim());
                if (!ok) throw new Error(`effects[${i}] 必须包含 type(legacy) 或 effectType(v3)`);
            }
        } catch (e) {
            this.setError(this.elErrEffects, 'effects JSON 无法解析/校验失败：' + e.message);
            this.updateSummary();
            return;
        }
